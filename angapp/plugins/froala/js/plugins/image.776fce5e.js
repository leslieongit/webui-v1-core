...
// Custom, endpointURL variable, grab server name from HTML
var server, server_loc;
var currentPath, parsedPath;
// Custom end
...
$.FE.PLUGINS.image = function (editor) {
        ...
        if (typeof data != 'undefined') {
          for (attr in data) {
            if (data.hasOwnProperty(attr)) {
              // Custom
              if (attr != "path_external" && attr != "path_internal") {
                // if (attr != 'link') {
                // Custom end
  ...
  function _parseResponse (response) {
    try {
      ...
      // Custom, adding image
      if (resp.path_external || resp.data.path_external) {
        return resp;
      }
      // if (resp.link) {
      //   return resp;
    ...
    try {
      if (editor.opts.imageUploadToS3) {
        ...
        if (status >= 200 && status < 300) {
          ...
          if (resp) {
            // Custom, adding our own image url
            var imageURL;
            if (parsedPath == "admin" || parsedPath == "campaign-setup" || parsedPath == "campaign-manager" || parsedPath == "rewards"){
              if (resp.path_external) {
                imageURL = server + "/static/images/" + resp.path_external;
              } else if (resp.data.path_external) {
                imageURL = server + "/static/images/" + resp.data.path_external;
              }
            }
            insert(imageURL, false, resp, $img, response || responseText);
            // Custom end
          }
        }
        else {
  ...
  function _addImage (link, data, loadCallback) {
    ...
    if (data && typeof data != 'undefined') {
      for (attr in data) {
        if (data.hasOwnProperty(attr)) {
          // custom
          if (attr != 'link' && attr != "path_internal") {
            //custom ends
  ...
  function upload (images, $image_placeholder) {
    ...
    if (typeof images != 'undefined' && images.length > 0) {
      ...
      if (form_data) {
        ...
        // Custom code for uploading image into our own endpoint
        // var xhr = editor.core.getXHR(url, editor.opts.imageUploadMethod);
        // _startUpload(xhr, form_data, image);
        server = $("textarea[data-server]").attr("data-server");
        server_loc = $("textarea[data-server]").attr("data-loc");
        currentPath = window.location.href.replace(document.getElementsByTagName('base')[0].href, ''); 
        parsedPath = currentPath.split("/")[0];

        // Create an XHR object
        var xhr = new XMLHttpRequest();
        // Create a FormData object for holding parameters
        var data = new FormData();
        data.append('success_action_status', '200');
        data.append('X-Requested-With', 'xhr');
        data.append('Content-Type', image.type);

        // Depends on the current path, decide what needs to be in parameter when sending XHR
        if (parsedPath == "admin") {
          // Define the endpoint we extract from the HTML
          var endpoint = server + server_loc + 'portal/resource/file';
          // Open a XMLHttpRequest
          xhr.open("POST", endpoint, true);
          // Pass in necessary parameters
          data.append('resource_type', 'file');
          data.append('resource_content_type', 'image');
          data.append('resource', image);
          data.append('region_id', '1');
        }
        else if (parsedPath == "campaign-setup" || parsedPath == "rewards" || parsedPath == "campaign-manager") {
          // Extracting the campaign ID out of the URL
          var campaignID;
          if (parsedPath == "campaign-manager"){
            campaignID = currentPath.split("/")[2];
          }
          else {
            campaignID = currentPath.split("/")[1];
          }
          var endpoint = server + server_loc + 'campaign/' + campaignID + '/resource/file';
          xhr.open("POST", endpoint, true);
          data.append("entry_id", campaignID);
          data.append('resource_type', 'file');
          data.append('resource_content_type', 'image');
          data.append('resource', image);
          data.append('region_id', '2');
        }
        // Sending requests to credentials in cross domain
        // This makes the XHR aware of HTTP cookies and HTTP Authentication information
        xhr.withCredentials = true;
        var $img = $current_image;

        var cur_user = readCookie('current.user');
        if(cur_user) {
          cur_user = JSON.parse(decodeURIComponent(cur_user));
          xhr.setRequestHeader('X-Auth-Token', cur_user['auth_token']);
        }
